...
def transfer_ddpg_graph_no_goal(env, ph, params):
	...
	for d_ in sorted(env.keys()):
		...
		# Learner policy
			...
			with tf.variable_scope('', reuse=tf.AUTO_REUSE):
				...
				graph[d_]['mapped_state']  = feedforward(in_node=ph[d_]['state'],
														 is_training=ph[d_]['is_training'],
		...
		# Learner policy
			...
			with tf.variable_scope('', reuse=tf.AUTO_REUSE):
				# Statemapping from learner to expert space
				mapped_state = feedforward(in_node=ph[d_]['next_state'],
										   is_training=ph[d_]['is_training'],
										   params=params[d_]['statemap'],
		...
		with tf.variable_scope('model', reuse=tf.AUTO_REUSE):
			...
			graph[d_]['model_raw_next_state']  = feedforward(in_node=sa_raw, is_training=ph[d_]['is_training'],
												 			 params=params[d_]['model'], scope=d_)
	...
	with tf.variable_scope('model', reuse=True):
		...
		# Next expert state
		trans_next_state = feedforward(in_node=sa_trans, is_training=ph[d_]['is_training'],
									   params=params[trans_d_]['model'], scope=trans_d_)


	# Map learner next state to expert space
	with tf.variable_scope('actor', reuse=True):
		mapped_next_state = feedforward(in_node=next_state, is_training=ph[d_]['is_training'],
